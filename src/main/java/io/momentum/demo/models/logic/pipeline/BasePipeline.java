...
import io.momentum.demo.models.logic.pipeline.options.FixedWindowOptions;
...
public abstract class BasePipeline {
  ...
  protected static PCollection<UserMessage> window(FixedWindowOptions options,
                                                   PCollection<UserMessage> stream) {
    return stream.apply(Window.<UserMessage>into(FixedWindows
                                                       .of(Duration.standardMinutes(options.getFixedWindowDuration())))
